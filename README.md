...
- **🤖 AI 3D 机器人**：基于 Three.js + WebGL 自主开发，史迪奇风格，通过代码构建
- **🐾 原创 EyePet 虚拟宠物**：纯 SVG + CSS 实现，眼睛实时追踪鼠标，随机眨眼，三种心情状态自动切换
- **📊 3D 算法可视化**：覆盖 21 种算法的 Three.js 3D 动态演示，v1.5 全面升级为真实曲面网格渲染
...
- **🎨 中国水墨画 UI**：纯 SVG 山水/云雾/墨竹/飞鸟/印章装饰，全站暖棕宣纸色调
- **🔐 多层安全防护**：API 异常捕获 + 输入校验 + 5 个 HTTP 安全头 + IP 速率限制
...
- AI 3D 机器人：Three.js + WebGL 原创实现，袋子风格，流畅动画与交互控制面板
- EyePet：纯 SVG 原创，眼睛追踪鼠标、随机眨眼、三种心情（开心/好奇/困倦）、弹跳漂浮动画

### 2. 3D 算法可视化系统 🆕 v1.5 升级
...
- **数学与几何**：正弦波、螺旋线、Mandelbrot 集(80×80/100迭代)、Lorenz 吸引子、3D 波浪、环面(TorusGeometry曲面+线框)、Klein 瓶(三角形曲面网格)
- **AI / ML 算法**：K-Means 聚类、线性回归、神经网络(3-6-4-2四层)、梯度下降、决策树(CanvasTexture中文标签)
...
### 5. AI 智能助手 🆕 悬浮升级 + DeepSeek V4-Flash

![AI聊天助手](https://github.com/partjava/i/blob/main/public/images/images/ai-chat.png)

集成 DeepSeek V4-Flash API，全站右下角悬浮窗，支持拖动/折叠，智能上下文识别，Markdown 渲染回复（代码语法高亮），一键总结保存为笔记，历史会话数据库持久化，Python FastAPI 服务 Docker 容器化独立部署。
...
纯 SVG 中国水墨画装饰（山水/云雾/墨竹/飞鸟/印章 6 种变体），覆盖全站 7 个页面，暖棕宣纸色调统一 UI，HeroSection 印章元素，FlipCard 宣纸纹理，现代 Web 技术与东方美学的融合。

### 13. 多层安全防护 🆕

6 个核心 API 路由 try/catch 异常处理，输入非空+长度校验，5 个 HTTP 安全响应头（HSTS/X-Frame-Options 等），middleware IP 维度速率限制（AI 30次/分、注册 5次/分、搜索 30次/分），全站 HTTPS 加密传输。

### 14. PWA 渐进式 Web 应用 🆕

Service Worker 离线缓存，manifest.json 完整配置（152/192/512px 图标），设置页"添加到主屏幕"按钮（iOS Safari 安装指引），通知权限弹窗，桌面安装体验接近原生 App。

### 15. 学习进度标记 🆕

study_progress 表持久化，右上角小圆钮默认折叠→点击展开，乐观更新+服务端同步，全局挂载至 study/layout.tsx 所有学习页面。

### 16. 暗黑模式 + 骨架屏 + Ctrl+K 🆕

body.dark 选择器覆盖 50+ 条全局规则，笔记/搜索页 loading.tsx 骨架屏，Ctrl+K 快捷键全局唤醒搜索。

### 17. 截图编辑器 🆕 v1.5.1

study 页面右下角相机按钮一键截图，全屏编辑器支持裁剪/画笔/方框/箭头/文字标注/粘贴外部图片/撤回重做/下载PNG/复制到剪贴板。

### 18. 安全密钥加固 🆕 v1.5.1

全量扫描泄露点，ecosystem.config.js 加入 .gitignore，DeepSeek 与 GLM API Key 全面轮换。
...
- **数据库**：MySQL 8.0 + 连接池
- **缓存**：Redis
- **AI 服务**：Python FastAPI + DeepSeek V4-Flash API（Docker 容器化）
