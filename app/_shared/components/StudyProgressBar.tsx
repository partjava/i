...
function useStudyCompletion() {
  ...
  // 每次路由变化时重新加载
  useEffect(() => {
    // 1) 先从 localStorage 乐观读取（解决刷新/重新进入后需重新标注的问题）
    const path = pathname;
    if (path) {
      try {
        const localVal = localStorage.getItem('study_completed_' + path);
        if (localVal === '1') {
          setCompleted(true);
        } else {
          setCompleted(false);
    ...
    // 2) 再从服务端拉取最新数据做校正（失败时保留 localStorage 状态）
    loadFromServer();
  }, [loadFromServer]);
...
export function StudyProgressBar({ inline = false }: { inline?: boolean }) {
  const { completed, toggleComplete, categoryProgress } = useStudyCompletion();
  const [collapsed, setCollapsed] = useState(true);

  if (inline) {
    return (
      <div className="bg-white/90 rounded-xl border border-black/5 shadow-sm p-3 flex flex-col gap-2">
        <span className="text-xs font-medium text-content-secondary px-1">学习进度</span>
        <button
          onClick={toggleComplete}
          className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium transition-all ${
            completed
              ? 'bg-brand-primary/15 text-brand-primary hover:bg-brand-primary/25 border border-brand-primary/30'
              : 'bg-surface-page text-content-primary hover:bg-surface-muted hover:text-content-primary border border-line-strong'
          }`}
        >
          {completed ? (
    ...
    <div className="fixed bottom-6 right-6 z-30">
      {collapsed ? (
        <button
          ...
          className="bg-surface-raised rounded-full shadow-lg border border-line-strong w-11 h-11 flex items-center justify-center hover:shadow-xl transition-shadow"
          title="学习进度"
        >
        ...
        <div className="bg-surface-raised rounded-xl shadow-lg border border-line-strong p-4 flex flex-col gap-3 min-w-[200px]">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-content-secondary">学习进度</span>
            <button onClick={() => setCollapsed(true)} className="text-content-muted hover:text-content-primary">
