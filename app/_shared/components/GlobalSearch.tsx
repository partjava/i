...
export default function GlobalSearch({ 
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/search/global?query=${encodeURIComponent(searchQuery)}`);
      const json = await response.json();

      if (json.success && json.data) {
        const items: SearchResult[] = [];
        (json.data.notes || []).forEach((note: any) => {
          items.push({ id: String(note.id), title: note.title, description: (note.content || '').slice(0, 120), category: note.category || '', type: 'note', path: `/notes/${note.id}`, score: 0.9 });
        });
        (json.data.challenges || []).forEach((ch: any) => {
          items.push({ id: String(ch.id), title: ch.title, description: ch.topicName || '', category: ch.topicName || '', type: 'course', path: '/challenges', score: 0.8 });
        });
  ...
  const handleSearch = (searchQuery: string) => {
    ...
    // 跳转到搜索结果页面
    router.push(`/tools/search?query=${encodeURIComponent(trimmedQuery)}`);
  };
