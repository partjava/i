...
const META: LessonMeta = {
  ...
  prevChapter: { label: '配置管理', href: '/study/security/ops/config' },
  nextChapter: { label: '灾难恢复', href: '/study/security/ops/recovery' },
  theme: THEMES.security,
}

const processCode = `import os
import json
import logging
import subprocess
...
const SPREADS = [
  {
    ...
    left: (
      <div className="space-y-4">
        <PageTitle>应急响应概述</PageTitle>
        <BookParagraph>
          应急响应是安全运维的重要工作，通过及时发现、分析和处置安全事件，降低安全风险，保护系统和数据安全。应急响应需要建立规范的流程，确保响应的及时性、有效性和可追溯性。
        </BookParagraph>
        <SectionTitle>核心工作</SectionTitle>
        <BookList
          items={[
            '建立应急响应机制',
            '制定响应流程和预案',
    ...
    right: (
      <div className="space-y-4">
        <SectionTitle>应急响应目标</SectionTitle>
        <BookList
          items={[
            '及时发现安全事件',
            '快速分析事件影响',
        ...
        />
        <BookDivider />
        <SectionTitle>关键要素</SectionTitle>
        <BookList
          items={[
            '专业的响应团队',
            '完善的预案体系',
  ...
  },
  {
    ...
    left: (
      <div className="space-y-4">
        <PageTitle>应急响应流程</PageTitle>
        <SectionTitle>准备阶段</SectionTitle>
        <BookList
          items={[
            '建立应急响应团队',
            '制定响应预案',
        ...
        />
        <SectionTitle>检测阶段</SectionTitle>
        <BookList
          items={[
            '监控系统告警',
            '分析异常行为',
        ...
        />
        <SectionTitle>分析阶段</SectionTitle>
        <BookList
          items={[
            '收集事件信息',
            '分析攻击路径',
        ...
        />
        <SectionTitle>处置阶段</SectionTitle>
        <BookList
          items={[
            '隔离受影响系统',
            '清除恶意代码',
        ...
        />
        <SectionTitle>恢复阶段</SectionTitle>
        <BookList
          items={[
            '验证系统安全',
            '恢复业务服务',
    ...
    right: (
      <div className="space-y-4">
        <PageTitle>响应流程管理</PageTitle>
        <BookParagraph>Python 应急响应流程管理脚本，实现从事件创建、分析、控制到恢复的完整闭环管理。</BookParagraph>
        <BookCode language="python" code={processCode} />
      </div>
    ),
  },
  {
    ...
    left: (
      <div className="space-y-4">
        ...
        />
        <SectionTitle>网络分析工具</SectionTitle>
        <BookList
          items={[
            'Wireshark',
            'tcpdump',
        ...
        />
        <SectionTitle>系统分析工具</SectionTitle>
        <BookList
          items={[
            'Volatility',
            'Autopsy',
        ...
        />
        <SectionTitle>恶意代码分析工具</SectionTitle>
        <BookList
          items={[
            'IDA Pro',
            'Ghidra',
    ...
    right: (
      <div className="space-y-4">
        ...
        <BookParagraph>
          日志分析：系统日志、应用日志、安全日志的收集与分析，发现异常行为和攻击痕迹。
        </BookParagraph>
        <BookParagraph>
          网络流量分析：抓包分析、流量溯源、异常流量检测，识别攻击路径和数据泄露。
        </BookParagraph>
        <BookParagraph>
          系统取证分析：内存、进程、文件、注册表等系统取证，定位攻击手法和后门。
        </BookParagraph>
        <BookParagraph>
          恶意代码分析：静态分析与动态分析结合，识别恶意代码行为和传播方式。
        </BookParagraph>
        <BookCode language="python" code={analysisCode} />
        <BookDivider />
        <PageTitle>工具管理脚本</PageTitle>
        <BookCode language="python" code={toolsCode} />
      </div>
    ),
  },
  {
    ...
    left: (
      <div className="space-y-4">
        <PageTitle>恢复措施</PageTitle>
        <BookParagraph>
          系统恢复：重装操作系统、恢复配置、修复受损文件。
        </BookParagraph>
        <BookParagraph>
          数据恢复：从备份恢复数据、修复数据库、验证数据完整性。
        </BookParagraph>
        <BookParagraph>
          服务恢复：重启服务、切换备用系统、恢复业务流程。
        </BookParagraph>
        <BookParagraph>
          安全加固：修补漏洞、加强访问控制、提升监控能力。
        </BookParagraph>
        <BookCode language="python" code={recoveryCode} />
      </div>
    ),
    right: (
      <div className="space-y-4">
        <PageTitle>实践案例</PageTitle>
        <SectionTitle>案例一：Web服务器DDoS攻击</SectionTitle>
        <BookList
          items={[
            '事件发现：监控发现流量异常，服务器响应缓慢',
            '事件分析：抓包分析流量，确认DDoS攻击，识别攻击源',
            '事件处置：封禁攻击IP，启用防护策略，调整网络配置',
            '系统恢复：重启服务，验证业务恢复，优化防护措施',
          ]}
        />
        <SectionTitle>案例二：数据库SQL注入攻击</SectionTitle>
        <BookList
          items={[
            '事件发现：日志发现异常SQL语句，数据被篡改',
            '事件分析：分析SQL日志，定位注入点，评估影响',
            '事件处置：修复代码漏洞，恢复数据备份，提升输入校验',
            '系统恢复：验证数据完整性，恢复服务，强化安全策略',
          ]}
        />
        <SectionTitle>案例三：勒索软件感染</SectionTitle>
        <BookList
          items={[
            '事件发现：文件被加密，出现勒索提示',
            '事件分析：分析感染范围，识别勒索软件类型',
            '事件处置：隔离受感染主机，清除恶意程序，恢复备份文件',
            '系统恢复：重建系统环境，恢复业务，完善备份和防护',
          ]}
        />
        <BookDivider />
        <PageTitle>案例管理脚本</PageTitle>
        <BookCode language="python" code={caseCode} />
      </div>
    ),
  },
]
