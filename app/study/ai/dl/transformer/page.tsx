...
const pytorchTransformerCode = `import torch
...
const tfTransformerCode = `import tensorflow as tf
...
const translationExerciseCode = `import torch
import torch.nn as nn
...
const summarizationCode = `import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import Dataset, DataLoader
import numpy as np
...
const META: LessonMeta = {
  ...
  prevChapter: { label: '注意力机制', href: '/study/ai/dl/attention' },
  nextChapter: { label: '生成对抗网络', href: '/study/ai/dl/gan' },
  theme: THEMES.ai,
}
...
const SPREADS = [
  {
    ...
    left: (
      <div className="space-y-4">
        <PageTitle>Transformer架构概述</PageTitle>

        <SectionTitle>基本结构</SectionTitle>
        <BookParagraph>
        ...
        <div className="mb-2">
          <svg width="100%" height="320" viewBox="0 0 800 320">
            {/* 编码器-解码器结构 */}
            <rect x="50" y="40" width="240" height="240" fill="#e2e8f0" stroke="#64748b" strokeWidth="2"/>
            <text x="170" y="30" textAnchor="middle" fill="#64748b">编码器</text>
            <rect x="450" y="40" width="240" height="240" fill="#e2e8f0" stroke="#64748b" strokeWidth="2"/>
            <text x="570" y="30" textAnchor="middle" fill="#64748b">解码器</text>
            {/* 编码器组件 */}
            <rect x="80" y="80" width="180" height="40" fill="#93c5fd" stroke="#3b82f6" strokeWidth="2"/>
            <text x="170" y="105" textAnchor="middle" fill="#3b82f6">多头自注意力</text>
            <rect x="80" y="140" width="180" height="40" fill="#93c5fd" stroke="#3b82f6" strokeWidth="2"/>
            <text x="170" y="165" textAnchor="middle" fill="#3b82f6">前馈神经网络</text>
            <rect x="80" y="200" width="180" height="40" fill="#93c5fd" stroke="#3b82f6" strokeWidth="2"/>
            <text x="170" y="225" textAnchor="middle" fill="#3b82f6">层归一化</text>
            {/* 解码器组件 */}
            <rect x="480" y="80" width="180" height="40" fill="#86efac" stroke="#22c55e" strokeWidth="2"/>
            <text x="570" y="105" textAnchor="middle" fill="#22c55e">掩码多头自注意力</text>
            <rect x="480" y="140" width="180" height="40" fill="#86efac" stroke="#22c55e" strokeWidth="2"/>
            <text x="570" y="165" textAnchor="middle" fill="#22c55e">编码器-解码器注意力</text>
            <rect x="480" y="200" width="180" height="40" fill="#86efac" stroke="#22c55e" strokeWidth="2"/>
            <text x="570" y="225" textAnchor="middle" fill="#22c55e">前馈神经网络</text>
            {/* 连接箭头 */}
            <path d="M290 160 L450 160" stroke="#64748b" strokeWidth="2" fill="none" markerEnd="url(#arrowhead)"/>
            <defs>
    ...
    right: (
      <div className="space-y-4">
        <SectionTitle>关键组件详解</SectionTitle>
        <BookParagraph>
        ...
        <SectionTitle>变体与改进</SectionTitle>
        <BookParagraph>
  ...
  },
  {
    ...
    left: (
      <div className="space-y-4">
        <PageTitle>Transformer实现</PageTitle>

        <SectionTitle>1. 使用PyTorch实现Transformer</SectionTitle>
        <BookCode language="python" code={pytorchTransformerCode} />
      </div>
    ),
    right: (
      <div className="space-y-4">
        <SectionTitle>2. 使用TensorFlow实现Transformer</SectionTitle>
        <BookCode language="python" code={tfTransformerCode} />
      </div>
    ),
  },
  {
    ...
    left: (
      <div className="space-y-4">
        ...
        <SectionTitle>题目一：机器翻译</SectionTitle>
        <BookParagraph><b>要求：</b>使用Transformer实现一个英汉翻译模型。</BookParagraph>
        <BookParagraph><b>参考答案：</b></BookParagraph>
        <BookCode language="python" code={translationExerciseCode} />
      </div>
    ),
    right: (
      <div className="space-y-4">
        <SectionTitle>题目二：文本摘要生成</SectionTitle>
        <BookParagraph><b>要求：</b>使用Transformer实现一个文本摘要生成模型。</BookParagraph>
        <BookParagraph><b>参考答案：</b></BookParagraph>
        <BookCode language="python" code={summarizationCode} />
      </div>
    ),
  },
]

export default function DlTransformerPage() {
