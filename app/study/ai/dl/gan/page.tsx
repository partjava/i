...
const dcganPytorchCode = `import torch
...
const wganTfCode = `import tensorflow as tf
...
const imageGenCode = `import torch
import torch.nn as nn
import torch.optim as optim
from torchvision import datasets, transforms
from torch.utils.data import DataLoader
...
const cycleganCode = `import torch
import torch.nn as nn
import torch.optim as optim
...
const META: LessonMeta = {
  subject: '深度学习',
  chapterTitle: '生成对抗网络(GAN)',
  chapterNumber: 7,
  totalChapters: 14,
  subjectHref: '/study/ai/dl',
  prevChapter: { label: 'Transformer架构', href: '/study/ai/dl/transformer' },
  nextChapter: { label: '自编码器', href: '/study/ai/dl/autoencoder' },
  theme: THEMES.ai,
}
...
const SPREADS = [
  {
      ...
      <div className="space-y-4">
        ...
        <div className="mb-2">
          <svg width="100%" height="240" viewBox="0 0 800 240">
            {/* GAN架构图 */}
            <rect x="50" y="40" width="240" height="160" fill="#e2e8f0" stroke="#64748b" strokeWidth="2"/>
            <text x="170" y="30" textAnchor="middle" fill="#64748b">生成器</text>
            <rect x="450" y="40" width="240" height="160" fill="#e2e8f0" stroke="#64748b" strokeWidth="2"/>
            <text x="570" y="30" textAnchor="middle" fill="#64748b">判别器</text>
            {/* 生成器组件 */}
            <rect x="80" y="80" width="180" height="40" fill="#93c5fd" stroke="#3b82f6" strokeWidth="2"/>
            <text x="170" y="105" textAnchor="middle" fill="#3b82f6">随机噪声输入</text>
            <rect x="80" y="140" width="180" height="40" fill="#93c5fd" stroke="#3b82f6" strokeWidth="2"/>
            <text x="170" y="165" textAnchor="middle" fill="#3b82f6">生成假样本</text>
            {/* 判别器组件 */}
            <rect x="480" y="80" width="180" height="40" fill="#86efac" stroke="#22c55e" strokeWidth="2"/>
            <text x="570" y="105" textAnchor="middle" fill="#22c55e">输入样本</text>
            <rect x="480" y="140" width="180" height="40" fill="#86efac" stroke="#22c55e" strokeWidth="2"/>
            <text x="570" y="165" textAnchor="middle" fill="#22c55e">真假判断</text>
            {/* 箭头 */}
            <path d="M290 160 L450 160" stroke="#64748b" strokeWidth="2" fill="none" markerEnd="url(#arrowhead)"/>
            <path d="M570 120 L480 120" stroke="#64748b" strokeWidth="2" fill="none" markerEnd="url(#arrowhead)"/>
            <defs>
    ...
    right: (
      <div className="space-y-4">
        <SectionTitle>训练过程</SectionTitle>
        <BookParagraph>
          GAN的训练过程是一个极小极大博弈：
        </BookParagraph>
        <BookList items={[
        ...
        <SectionTitle>GAN变体</SectionTitle>
        <BookParagraph>
          GAN的几种主要变体：
        </BookParagraph>
        <BookList items={[
  ...
  },
  {
    ...
    left: (
      <div className="space-y-4">
        <PageTitle>GAN实现</PageTitle>

        <SectionTitle>1. 使用PyTorch实现DCGAN</SectionTitle>
        <BookCode language="python" code={dcganPytorchCode} />
      </div>
    ),
    right: (
      <div className="space-y-4">
        <SectionTitle>2. 使用TensorFlow实现WGAN</SectionTitle>
        <BookCode language="python" code={wganTfCode} />
      </div>
    ),
  },
  {
    ...
    left: (
      <div className="space-y-4">
        ...
        <SectionTitle>题目一：图像生成</SectionTitle>
        <BookParagraph><b>要求：</b>使用DCGAN生成手写数字图像。</BookParagraph>
        <BookParagraph><b>参考答案：</b></BookParagraph>
        <BookCode language="python" code={imageGenCode} />
      </div>
    ),
    right: (
      <div className="space-y-4">
        <SectionTitle>题目二：风格迁移</SectionTitle>
        <BookParagraph><b>要求：</b>使用CycleGAN实现图像风格迁移。</BookParagraph>
        <BookParagraph><b>参考答案：</b></BookParagraph>
        <BookCode language="python" code={cycleganCode} />
      </div>
    ),
  },
]

export default function DlGanPage() {
